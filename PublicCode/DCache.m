...
@interface DCachePolicy()

@end

@implementation DCachePolicy

+(id)cachePolicyWithCacheType:(DCacheType)cacheType {
    return [[DCachePolicy alloc] initWithCacheType:cacheType];
}
...
-(id)copyWithZone:(NSZone *)zone {
    DCachePolicy* cachePolicy = [[DCachePolicy allocWithZone:zone] initWithCacheType:_cacheType];
    return cachePolicy;
}
...
+ (NSString *)fileCachePath:(DCacheType)cacheType {
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentDirectory = [paths objectAtIndex:0];
...
+ (void)addCache:(NSString *)key obj:(id <NSCoding>)obj {
    [self addCache:key obj:obj type:DCacheTypeNone];
}
...
+ (void)addCache:(NSString *)key obj:(id <NSCoding>)obj type:(DCacheType)type expDate:(NSDate *)expDate {
    DCachePolicy* policy = [[DCachePolicy alloc] initWithCacheType:type];
